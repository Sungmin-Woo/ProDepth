...
import math
import torch.cuda
#from ProDepth.networks.prior_block import PriorBlock
from ProDepth.layers import ConvBlock, Conv3x3, upsample

class LiteCVEncoder(nn.Module):
    """
    Lite-Mono
    """
    ...
    def forward_features(self, x):
        # mono_feature_0 [B,C(64),H/2,W/2]
        # mono_feature_1 [B,C(64),H/4,W/4]
        # cv_features [B,BINS(128),H/4,W/4]
        ...
        return tmp_x, x_down, features
    
    def forward(self, tmp_x, x_down, features, cv_feature):
        
        fcv = self.up0(torch.cat([tmp_x[-1],cv_feature],1))
        tmp_x[-1] = fcv
        features[-1] = fcv
...
class LiteMono(nn.Module):
    ...
    def forward_features2(self, tmp_x, x_down, features):
        for i in range(1, 3):
            tmp_x.append(x_down[i])
            x = torch.cat(tmp_x, dim=1)
            x = self.downsample_layers[i](x)
    ...
    def forward(self, x):
        tmp, x, features = self.forward_features(x)
        x = self.forward_features2(tmp, x, features)
