...
class Trainer:
    def __init__(self, options):
        ...
        if self.train_teacher_and_pose:
            ...
            if self.opt.dataset =="kitti":
                self.min_depth_tracker = 0.12
                self.max_depth_tracker = 6.92
            else:
                self.min_depth_tracker = 0.13
                self.max_depth_tracker = 5.17   
        ...
        self.models["encoder"] = networks.ResnetEncoderMatching(
            self.opt.num_layers, self.opt.weights_init == "pretrained", self.opt.encoder,
        ...
        self.models["mono_encoder"] = \
            networks.LiteMono(model='lite-mono-8m', drop_path_rate=0.4)
        ...
        self.models["mono_depth"] = \
            networks.DepthDecoder(self.models["mono_encoder"].num_ch_enc, [0,1,2]) # self.opt.scales
        if self.opt.ddp:
        ...
        self.models["mono_uncert"] = \
            networks.DepthDecoderUncert(self.models["mono_encoder"].num_ch_enc, [0,1,2])
        if self.opt.ddp:
            self.models["mono_uncert"] = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.models["mono_uncert"])
        self.models["mono_uncert"].to(self.device)
        ...
        if self.train_teacher_and_pose:
            ...
            self.parameters_to_train += list(self.models["mono_uncert"].parameters())
    ...
    def process_batch(self, inputs, is_train=False):
        ...
        if self.train_teacher_and_pose:
            ...
            with torch.no_grad():
                feats = self.models["mono_encoder"](inputs["color_aug", 0, 0])
                mono_outputs.update(self.models['mono_depth'](feats))
                mono_outputs.update(self.models['mono_uncert'](feats))
        ...
        for key in list(mono_outputs.keys()):
            _key = list(key)
            if _key[0] in ['depth', 'disp', 'uncert']:
                _key[0] = 'mono_' + key[0]
                _key = tuple(_key)
                outputs[_key] = mono_outputs[key]

        features, lowest_cost, confidence_mask, cv_features, cost_volume_refine_mask, refined_lowest_cost, gaussian_cost = self.models["encoder"](inputs["color_aug", 0, 0],
                                                                        lookup_frames,
                                                                        relative_poses,
                                                                        inputs[('K', 2)],
                                                                        inputs[('inv_K', 2)],
                                                                        ...
                                                                        mono_disp = outputs[('mono_disp', 0)],
                                                                        var = outputs[('mono_uncert',0)] * self.opt.mono_var_scale,
                                                                        )   

        outputs[("cvf_disp",0)] = cv_features        
        self.cvf_generate_images_pred(inputs,outputs,is_multi=False)

        outputs.update(self.models["depth"](features))
    
        #outputs["cost_volume_refine_mask"] = cost_volume_refine_mask
        #outputs["lowest_cost"] = F.interpolate(lowest_cost.unsqueeze(1),
        #                                       [self.opt.height, self.opt.width],
        #                                       mode="nearest")[:, 0] 
        #outputs["refined_lowest_cost"] = F.interpolate(refined_lowest_cost.unsqueeze(1),
        #                                       [self.opt.height, self.opt.width],
        #                                       mode="nearest")[:, 0]   
        #outputs["gaussian_cost"] = F.interpolate(gaussian_cost.unsqueeze(1),
        #                                       [self.opt.height, self.opt.width],
        #                                       mode="nearest")[:, 0]            

        if not self.opt.disable_motion_masking:
            outputs["cvf_mask"] = self.compute_cvf_mask_weight(outputs)
        ...
        cvf_losses = self.compute_cvf_losses(inputs, outputs, is_multi=True)
        for key, val in cvf_losses.items():
                losses[key] +=0.3*val
    ...
    def cvf_generate_images_pred(self, inputs, outputs, is_multi=False):
            """Generate the warped (reprojected) color images for a minibatch.
            Generated images are saved into the `outputs` dictionary.
            """
            scale = 0
            disp = outputs[("cvf_disp", scale)].unsqueeze(1)
            if self.opt.v1_multiscale:
                source_scale = scale
            else:
                source_scale = 0
            _, depth = disp_to_depth(disp, self.opt.min_depth, self.opt.max_depth)
            outputs[("cvf_depth", 0, scale)] = depth
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                T = outputs[("cam_T_cam", 0, frame_id)]
                if is_multi:
                    # don't update posenet based on multi frame prediction
                ...
                pix_coords = self.project_3d[source_scale](
                    cam_points, inputs[("K", source_scale)], T)
                outputs[("cvf_sample", frame_id, scale)] = pix_coords
                outputs[("cvf_color", frame_id, scale)] = F.grid_sample(
                    inputs[("color", frame_id, source_scale)],
                    outputs[("cvf_sample", frame_id, scale)],
                    padding_mode="border", align_corners=True)
    ...
    def compute_cvf_mask_weight(self, outputs):
            """Generate a mask of where we cannot trust the cost volume, based on the difference
            between the cost volume and the teacher, monocular network"""

            mono_disp, mono_depth = disp_to_depth(outputs[('mono_disp', 0)].detach(),self.opt.min_depth, self.opt.max_depth)
            matching_disp, matching_depth = disp_to_depth(outputs[('cvf_disp',0)].unsqueeze(1).detach(),self.opt.min_depth, self.opt.max_depth)

            depth_mask = torch.exp(-torch.abs(matching_depth - mono_depth)*(2/3))
            disp_mask = torch.exp(-torch.abs(matching_disp - mono_disp)*(2/3))

            weighted_mask =  disp_mask * depth_mask
            binary_mask = (weighted_mask > 0.4).float()

            mask =  weighted_mask * binary_mask
    ...
    def compute_losses(self, inputs, outputs, is_multi=False):
        ...
        #for scale in scales:
            ...
            # consistency loss to
            reprojection_loss_mask = (reprojection_loss_mask * outputs["cvf_mask"].unsqueeze(1)) 
            if is_multi:
            ...
            if is_multi:
                multi_depth, _ =  disp_to_depth(outputs[("disp", 0)], self.opt.min_depth, self.opt.max_depth)
                mono_depth, _ =  disp_to_depth(outputs[("mono_disp", 0)].detach(), self.opt.min_depth, self.opt.max_depth)

                consistency_loss = torch.abs(multi_depth - mono_depth) * consistency_mask
                consistency_loss = consistency_loss.mean() * 0.1 
    ...
    def compute_cvf_losses(self, inputs, outputs, is_multi=False):
            """Compute the reprojection, smoothness and proxy supervised losses for a minibatch
            """
            ...
            if self.opt.v1_multiscale:
                ...
                source_scale = 0
            disp = outputs[("cvf_disp", scale)]
            color = inputs[("color", 0, scale)]
            target = inputs[("color", 0, source_scale)]
            for frame_id in self.opt.frame_ids[1:]:
                pred = outputs[("cvf_color", frame_id, scale)]
                reprojection_losses.append(self.compute_reprojection_loss(pred, target))
            reprojection_losses = torch.cat(reprojection_losses, 1)
            if self.opt.avg_reprojection:
                reprojection_loss = reprojection_losses.mean(1, keepdim=True)
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            ...
            disp = colormap(outputs[('cvf_disp', s)][j])
            writer.add_image(
                "disp_cvf/{}".format(j),
                disp, self.step)

            var = colormap(outputs[('mono_uncert', s)][j]).squeeze(0)
            writer.add_image(
                "var/{}".format(j),
                var, self.step)
            ...
            if outputs.get("lowest_cost") is not None:
                ...
                refined_lowest_cost = outputs["refined_lowest_cost"][j]
                gaussian_cost = outputs["gaussian_cost"][j]
                ...
                cvf_mask = \
                    outputs['cvf_mask'][j].cpu().detach().unsqueeze(0).numpy()    
                writer.add_image(
                    "cvf_mask/{}".format(j),
                    cvf_mask, self.step)
        '''
    ...
    def load_mono_model(self):
        ...
        model_list = ['pose_encoder', 'pose', 'mono_encoder', 'mono_depth','mono_uncert']
        for n in model_list:
            if self.local_rank == 0:
