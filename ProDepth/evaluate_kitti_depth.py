...
def evaluate(opt):
    ...
    if opt.encoder == "lite" or opt.encoder=="lite_prior":
        opt.scales = [0,1,2]
    else:
        opt.scales = [0,1,2,3]
    ...
    if opt.ext_disp_to_eval is None:
        ...
        mono_decoder_uncert_path = os.path.join(opt.load_weights_folder, "mono_uncert.pth")


        #if opt.encoder == "lite" or opt.encoder=="lite_prior":
        #    mono_encoder_class = networks.LiteMono
        #else:
        #    mono_encoder_class = networks.ResnetEncoder
        mono_encoder_class = networks.LiteMono


        multi_encoder_path = os.path.join(opt.load_weights_folder, "encoder.pth")
        multi_decoder_path = os.path.join(opt.load_weights_folder, "depth.pth")
        ...
        try:
            HEIGHT, WIDTH = multi_encoder_dict['height'], multi_encoder_dict['width']
        except KeyError:
        ...
        # setup models
        #if opt.encoder =="lite" or opt.encoder=="lite_prior":
        #    mono_encoder_opts = dict(model='lite-mono-8m', drop_path_rate=0.4)
        #else:
        #    mono_encoder_opts = dict(num_layers=opt.num_layers,
        #                            pretrained=False)
        mono_encoder_opts = dict(model='lite-mono-8m', drop_path_rate=0.4)
        
        multi_encoder_opts = dict(num_layers=opt.num_layers,
                            pretrained=False,
                            encoder = opt.encoder,
        ...
        min_depth_bin = multi_encoder_dict.get('min_depth_bin')
        max_depth_bin = multi_encoder_dict.get('max_depth_bin')
        ...
        multi_depth_decoder = networks.DepthDecoder(multi_encoder.num_ch_enc, scales = opt.scales)
        mono_depth_decoder = networks.DepthDecoder(mono_encoder.num_ch_enc, scales = [0,1,2])
        mono_depth_uncert_decoder = networks.DepthDecoderUncert(mono_encoder.num_ch_enc, scales = [0,1,2])
        ...
        mono_depth_uncert_decoder.eval()
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                mono_output2 = mono_depth_uncert_decoder(mono_feat)
                
                mono_pred_disp, mono_pred_depth = disp_to_depth(mono_output[("disp", 0)], opt.min_depth, opt.max_depth)
                mono_pred_disp = mono_pred_disp.cpu()[:, 0].numpy()
                ...
                multi_output, lowest_cost, costvol, _, __, ___, _____ = multi_encoder(input_color, lookup_frames,
                                        relative_poses,
                                        K,
                                        invK,
                                        min_depth_bin, max_depth_bin,
                                        mono_disp = mono_output[("disp", 0)],
                                        var = mono_output2[('uncert',0)]*opt.mono_var_scale)
                multi_output = multi_depth_decoder(multi_output)
                multi_pred_disp, _ = disp_to_depth(multi_output[("disp", 0)], opt.min_depth, opt.max_depth)
                multi_pred_disp = multi_pred_disp.cpu()[:, 0].numpy()
                multi_pred_disps.append(multi_pred_disp)


        multi_pred_disps = np.concatenate(multi_pred_disps)
    ...
    for i in tqdm.tqdm(range(multi_pred_disps.shape[0])):
        ...
        multi_pred_disp = np.squeeze(multi_pred_disps[i])
        multi_pred_disp = cv2.resize(multi_pred_disp, (gt_width, gt_height))
        multi_pred_depth = 1 / multi_pred_disp
        ...
        multi_pred_depth = multi_pred_depth[mask]

        gt_depth = gt_depth[mask]

        #mono_pred_depth *= opt.pred_depth_scale_factor
        multi_pred_depth *= opt.pred_depth_scale_factor

        if not opt.disable_median_scaling:
    
            multi_ratio = np.median(gt_depth) / np.median(multi_pred_depth)
            multi_ratios.append(multi_ratio)
            multi_pred_depth *= multi_ratio

        multi_pred_depth[multi_pred_depth < MIN_DEPTH] = MIN_DEPTH
        multi_pred_depth[multi_pred_depth > MAX_DEPTH] = MAX_DEPTH

        multi_errors.append(compute_errors(gt_depth, multi_pred_depth ))
    ...
    if opt.save_pred_disps:
        ...
        np.save(output_path, np.array(multi_errors))

    if not opt.disable_median_scaling:
        multi_ratios = np.array(multi_ratios)
        med = np.median(multi_ratios)
        print("Mutli Scaling ratios | med: {:0.3f} | std: {:0.3f}".format(med, np.std(multi_ratios / med)))

    mean_errors = np.array(multi_errors).mean(0)
