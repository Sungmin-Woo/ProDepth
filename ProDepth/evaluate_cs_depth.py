...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        multi_depth_decoder = networks.DepthDecoder(multi_encoder.num_ch_enc, scales = [0,1,2])
        ...
        with torch.no_grad():
            for i, data in tqdm.tqdm(enumerate(dataloader)):
                ...
                multi_output, lowest_cost, costvol, _, __, ____, _____ = multi_encoder(input_color, lookup_frames,
                                        relative_poses,
                                        K,
                                        invK,
    ...
    for i in tqdm.tqdm(range(multi_pred_disps.shape[0])):
        ...
        if opt.eval_split == 'cityscapes':
            ...
            multi_pred_depth = multi_pred_depth[256:, 192:1856]
    ...
    if opt.save_pred_disps:
        ...
        #np.save(output_path, np.array(mono_errors))
    ...
    if not opt.disable_median_scaling:
        ...
        print(" MULTI Scaling ratios | med: {:0.3f} | std: {:0.3f}".format(med, np.std(multi_ratios / med)))
